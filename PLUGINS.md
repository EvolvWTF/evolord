...
This document explains how to build Evolord plugins, how the runtime works, and what you can do from a plugin UI and a WASM module.

> TL;DR: A plugin is a zip file with 4 files at the root: `<id>.wasm`, `<id>.html`, `<id>.css`, `<id>.js`. Upload it in the Plugins page or drop it in Evolord-Server/plugins.
...
```
Evolord-Server/plugins/<pluginId>/
  ├─ <pluginId>.wasm
  ├─ manifest.json
  └─ assets/
     ├─ <pluginId>.html
...
- Or drop `<pluginId>.zip` into Evolord-Server/plugins and restart the server
...
Evolord plugins have **two parts**:
...
- Evolord-Server/public/remotedesktop.html
- Evolord-Server/public/assets/remotedesktop.js
