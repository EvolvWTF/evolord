...
func Load() Config {
	server := strings.TrimSpace(os.Getenv("EVOLORD_SERVER"))
	if server == "" {
		server = DefaultServerURL
	}

	rawServerFlag := strings.TrimSpace(os.Getenv("EVOLORD_SERVER_RAW"))
	if rawServerFlag == "" {
	...
	if v := strings.TrimSpace(os.Getenv("EVOLORD_CAPTURE_INTERVAL")); v != "" {
	...
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("EVOLORD_DISABLE_CAPTURE"))); v != "" {
		disableCapture = v == "true" || v == "1" || v == "yes"
	}

	enablePersistence := strings.ToLower(DefaultPersistence) == "true"
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("EVOLORD_ENABLE_PERSISTENCE"))); v != "" {
		enablePersistence = v == "true" || v == "1" || v == "yes"
	}

	tlsInsecureSkipVerify := true
	if v := strings.ToLower(strings.TrimSpace(os.Getenv("EVOLORD_TLS_INSECURE_SKIP_VERIFY"))); v != "" {
		tlsInsecureSkipVerify = v == "true" || v == "1" || v == "yes"
	}
	tlsCAPath := strings.TrimSpace(os.Getenv("EVOLORD_TLS_CA"))
	tlsClientCert := strings.TrimSpace(os.Getenv("EVOLORD_TLS_CLIENT_CERT"))
	tlsClientKey := strings.TrimSpace(os.Getenv("EVOLORD_TLS_CLIENT_KEY"))

	mutex := strings.TrimSpace(os.Getenv("EVOLORD_MUTEX"))
	if mutex == "" {
		mutex = DefaultMutex
	}
