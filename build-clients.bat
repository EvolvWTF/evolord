...
setlocal
set ROOT=%~dp0
set CLIENT_DIR=%ROOT%Evolord-Client
...
if "%ENABLE_PERSISTENCE%"=="true" (
    ...
    set "LDFLAGS=%LDFLAGS% -X evolord-client/cmd/agent/config.DefaultPersistence=true"
)

if not "%SERVER_URL%"=="" (
    echo Building with custom server URL: %SERVER_URL%
    set "LDFLAGS=%LDFLAGS% -X evolord-client/cmd/agent/config.DefaultServerURL=%SERVER_URL%"
)

if not "%CLIENT_ID%"=="" (
    echo Building with custom client ID: %CLIENT_ID%
    set "LDFLAGS=%LDFLAGS% -X evolord-client/cmd/agent/config.DefaultID=%CLIENT_ID%"
)

if not "%CLIENT_COUNTRY%"=="" (
    echo Building with custom country: %CLIENT_COUNTRY%
    set "LDFLAGS=%LDFLAGS% -X evolord-client/cmd/agent/config.DefaultCountry=%CLIENT_COUNTRY%"
)
...
set GOARCH=386
set CGO_ENABLED=0
%BUILD_CMD% -ldflags="%LDFLAGS% %WIN_LDFLAGS%" -o "%OUT_DIR%\agent-windows-386.exe" ./cmd/agent
if errorlevel 1 goto :err
...
set GOARCH=amd64
set CGO_ENABLED=0
%BUILD_CMD% -ldflags="%LDFLAGS%" -o "%OUT_DIR%\agent-darwin-amd64" ./cmd/agent
if errorlevel 1 goto :err
