...
def resolve_default_db() -> str:
    ...
    repo_default = os.path.abspath(os.path.join(here, "..", "data", "evolord.db"))
    return repo_default if os.path.exists(os.path.dirname(repo_default)) else "evolord.db"


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Seed fake clients into evolord.db for load testing."
    )
    parser.add_argument(
        "--db",
        default=resolve_default_db(),
        help="Path to evolord.db (default: Evolord-Server/data/evolord.db if present)",
    )
    parser.add_argument(
        "--count",
        type=int,
